...
import fit.quanlyspa.service.RedisTokenService;
...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(
        ...
        try {
            ...
            if (!jwtTokenProvider.isValidTokenFormat(jwt)) {
                log.warn("Invalid JWT format for request: {}", request.getRequestURI());
                filterChain.doFilter(request, response);
                return;
            }
            ...
            if (redisTokenService.isTokenBlacklisted(jti)) {
                log.warn("Blacklisted token attempt for jti: {}", jti);
                filterChain.doFilter(request, response);
                return;
            }
